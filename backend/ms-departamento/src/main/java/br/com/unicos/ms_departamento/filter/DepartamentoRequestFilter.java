package br.com.unicos.ms_departamento.filter;
...
public class DepartamentoRequestFilter extends OncePerRequestFilter {
    ...
    private void authenticateRequest(HttpServletRequest request, String authorizationHeader) {
        ...
        String path = request.getServletPath();
        if(path.startsWith("/v1/departamentos"))
            verificarPermissao(request.getMethod(), "DEPARTAMENTO_");
        else if(path.startsWith("/v1/responsaveis-departamento"))
            verificarPermissao(request.getMethod(), "DEPARTAMENTO_RESPONSAVEL_");
        else if(path.startsWith("/v1/vinculos-departamento-filial"))
            verificarPermissao(request.getMethod(), "DEPARTAMENTO_VINCULO_FILIAL_");
