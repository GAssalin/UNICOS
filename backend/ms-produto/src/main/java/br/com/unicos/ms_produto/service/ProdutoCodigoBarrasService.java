...
public class ProdutoCodigoBarrasService extends BaseTenantService<ProdutoCodigoBarras, Long> {
    ...
    @CircuitBreaker(name = "produto-codigo-barras-admin", fallbackMethod = "fallbackAdmin")
    ...
    @Transactional(readOnly = true)
    @CircuitBreaker(name = "produto-codigo-barras-admin", fallbackMethod = "fallbackAdminList")
    ...
    @CircuitBreaker(name = "produto-codigo-barras-admin", fallbackMethod = "fallbackAdminVoid")
    public void remover(Long id) {
        Long empresaId = TenantContext.getEmpresaId();
        repository.delete(buscarCodigo(id, empresaId));
    }

    @CircuitBreaker(name = "produto-codigo-barras-admin", fallbackMethod = "fallbackAdminVoid2")
