...
import br.com.unicos.ms_pagamentos.mapper.ProdutoMapper;
import br.com.unicos.ms_pagamentos.model.Produto;
import br.com.unicos.ms_pagamentos.repository.ProdutoRepository;
...
public class ProdutoService extends BaseTenantService<Produto, Long> {
    ...
    @CircuitBreaker(name = "pagamentos-produto-admin", fallbackMethod = "fallbackAdmin")
    ...
    @Transactional(readOnly = true)
    @CircuitBreaker(name = "pagamentos-produto-admin", fallbackMethod = "fallbackAdminPage")
    ...
    @CircuitBreaker(name = "pagamentos-produto-admin", fallbackMethod = "fallbackAdminVoid")
    ...
    private ProdutoResponse fallbackAdmin(Object req, Throwable ex) {
        throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "Serviço de pagamentos temporariamente indisponível");
    }
