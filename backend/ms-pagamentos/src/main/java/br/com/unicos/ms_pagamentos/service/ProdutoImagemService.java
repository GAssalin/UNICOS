...
import br.com.unicos.ms_pagamentos.mapper.ProdutoImagemMapper;
import br.com.unicos.ms_pagamentos.model.ProdutoImagem;
import br.com.unicos.ms_pagamentos.repository.ProdutoImagemRepository;
...
public class ProdutoImagemService extends BaseTenantService<ProdutoImagem, Long> {
    ...
    @CircuitBreaker(name = "pagamentos-produto-imagem-admin", fallbackMethod = "fallbackAdmin")
    ...
    @Transactional(readOnly = true)
    @CircuitBreaker(name = "pagamentos-produto-imagem-admin", fallbackMethod = "fallbackAdminPage")
    ...
    @CircuitBreaker(name = "pagamentos-produto-imagem-admin", fallbackMethod = "fallbackAdminVoid")
    public void remover(Long id) {
        Long empresaId = TenantContext.getEmpresaId();
        repository.delete(buscarImagem(id, empresaId));
    }

    @CircuitBreaker(name = "pagamentos-produto-imagem-admin", fallbackMethod = "fallbackAdminVoid2")
