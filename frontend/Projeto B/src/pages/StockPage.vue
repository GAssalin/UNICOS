<template>
  <section class="module-page">
    ...
    <div class="results-card">
      ...
      <div class="table-wrap"><table class="data-table"><thead><tr><th>Código</th><th>Produto</th><th>Categoria</th><th>Fornecedor</th><th>Saldo</th><th>Mínimo</th><th>Valor em estoque</th><th>Status</th></tr></thead><tbody><tr v-for="product in filteredProducts" :key="product.id"><td>{{ product.codigo }}</td><td><strong>{{ product.nome }}</strong></td><td>{{ product.categoria || '-' }}</td><td>{{ product.fornecedor || '-' }}</td><td>{{ product.saldo }} {{ product.unidade }}</td><td>{{ product.estoqueMinimo }} {{ product.unidade }}</td><td>{{ currency(product.valorEstoque) }}</td><td><span :class="['stock-pill', product.stockStatus.className]">{{ product.stockStatus.label }}</span><small class="stock-description">
    {{ product.stockStatus.description }}</small></td></tr></tbody></table></div>
    </div>
  </section>
</template>
<script setup>
